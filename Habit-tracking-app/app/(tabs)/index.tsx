...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const habbitsSubscription = client.subscribe(
      habbitChannel,
      (response: RealtimeResponse) => {
        if (response.events.includes("databases.*.collections.*.documents.*.created"))
        {
          fetchHabits();
        }else if (
          response.events.includes("databases.*.collections.*.documents.*.updated")
        ) {
          fetchHabits();
        }else if(
          response.events.includes("databases.*.collections.*.documents.*.deleted")
        ){
          fetchHabits();
        }
      }
   ...
   const handleDeleteHabit = async ( id: string)=>{
    ...
    try{
      await databases.createDocument(DATABASE_ID, HABBIT_COMPLETIONS_COLLECTION_ID, ID.unique(),{
        habit_id: id,
        user_id: user?.$id,
...
const isHabbitCompleted = (id: string) => {
