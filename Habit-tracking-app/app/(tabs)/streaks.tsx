...
export default function StreaksScreen() {
  ...
  interface StreakData {
    ...
    const getStreakData = (habitId: string): StreakData => {
        const habitCompletions = completeHabits?.filter((c) => c.habit_id === habitId)
            .sort(
                (a, b) => new Date(a.completed_at).getTime() - 
    ...
    habitCompletions?.forEach((c) => {
        ...
        if (lastsDate) {
            const diff =
             (date.getTime() - lastsDate.getTime()) / (1000 * 60 * 60 * 24);
